...
- `progress`: 진행 중일 때만 값이 있고 `done`·`failed`면 `null`.
  화면 표기는 `collecting`이면 "수집 중 · 23 / 198", `parsing`이면 "파싱 · 65%"
  (퍼센트는 프론트가 `current/total`로 계산한다).
  **진행률이 세는 대상은 단계마다 바뀐다** — 커밋 저장 중에는 커밋 수, PR 수집 중에는 PR 수,
  파싱 중에는 파일 수다. 앞의 둘이 모두 `collecting` 안에서 일어나므로
  화면에 "커밋"처럼 단위를 붙이면 틀린 값이 된다. 숫자만 보여준다.
